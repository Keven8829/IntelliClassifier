...
def drawing_window():
    class Drawing():
        ...
        def painting(self, event): # Painting on canvas
            ...
            self.canvas.create_oval(x1, y1, x2, y2, fill=self.pen_colour, width=self.pen_scale.get(), smooth=True, capstyle="round")
            self.draw.line((x1, y1, x2, y2), fill=self.pen_colour, width=self.pen_scale.get())

        def paint(self,e):
            if self.old_x and self.old_y:
                self.canvas.create_line(self.old_x,self.old_y,e.x,e.y,width=self.pen_scale.get(),fill=self.pen_colour,capstyle='round',smooth=True)
                self.draw.line((self.old_x, self.old_y, e.x, e.y), fill=self.pen_colour, width=self.pen_scale.get())
        ...
        def statistics(self):
            ...
            # Create a new Tkinter window for statistics
            ...
            fig_loss , fig_accuracy = show_stats()
        ...
        def log(self): # Log
            ...
            self.log_text.config(text=self.message)
            self.log_text.pack()
            # self.log_text = tk.Text(self.log_frame, wrap="word", font=("Times New Roman", 12), bg="white", height=15, width=33)
            # self.log_text.pack()
            # self.log_text.insert(tk.END, "test")
        ...
        def result(self): # Result
            ...
            # self.result_text = tk.Text(self.result_frame, wrap="word", font=("Times New Roman", 12), bg="white", height=2, width=33)
            # self.result_text.pack()
            # self.result_text.insert(tk.END, "test")
